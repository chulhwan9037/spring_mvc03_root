...
<body>
	...
	<form method="post" enctype="multipart/form-data">
		...
		<tbody>
			...
			<tr>
				<th>내용</th>
				<td align="left"><textarea rows="10" cols="60" name="content" value="${bovo.content}"></textarea>
				</td>
			</tr>
			<tr>
				<th>첨부파일</th>
				<c:choose>
					<c:when test="${empty bovo.f_name}">
						<td align="left">
							<input type="file" name="file"><b>이전파일 없음</b>
							<input type="hidden" name="old_f_name" value="">
						</td>						
					</c:when>
					<c:otherwise>
							<input type="file" name="file"><b>${bovo.f_name}</b>
							<input type="hidden" name="old_f_name"><b>${bovo.f_name}</b>
					</c:otherwise>
				</c:choose>
			</tr>
			<tr>
